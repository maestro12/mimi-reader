...
public interface FourChanApi {
    @GET("boards.json")
    Single<FourChanBoards> fetchBoards();

    @GET("{boardName}/{pageNumber}.json")
    Single<FourChanThreads> fetchPage(@Path("pageNumber") int page, @Path("boardName") String boardName);
    ...
    Single<FourChanThread> fetchThread(@Path("boardName") String boardName, @Path("threadId") long threadId, @Header("Cache-Control") String cacheControl);
